...
Micro.lerp = function (a, b, percent) {
    return a + (b - a) * percent;
};
Micro.rand = function (a, b) {
    return Micro.lerp(a, b, Math.random());
};
Micro.randInt = function (a, b, p) {
    return Micro.lerp(a, b, Math.random()).toFixed(p || 0) * 1;
}
        ...
        for (x = 0; x < W; x++) {
            for (y = 0; y < H; y++) {
                if (this.map.getTileValue(x, y) === Tile.REDGE) {
                    ...
                    if (temp !== Tile.RIVER && Random.getRandom(2)) {
                        temp++;
                    }
...
Micro.isCommercial = Micro.unwrapTile(function (tile) {
...
Micro.isDriveable = Micro.unwrapTile(function (tile) {
    return (tile >= Tile.ROADBASE && tile <= Tile.LASTRAIL) || tile === Tile.RAILHPOWERV || tile === Tile.RAILVPOWERH;
});
Micro.isFire = Micro.unwrapTile(function (tile) {
    return tile >= Tile.FIREBASE && tile < Tile.ROADBASE;
});
Micro.isFlood = Micro.unwrapTile(function (tile) {
    return tile >= Tile.FLOOD && tile < Tile.LASTFLOOD;
});
Micro.isIndustrial = Micro.unwrapTile(function (tile) {
    return tile >= Tile.INDBASE && tile < Tile.PORTBASE;
});
Micro.isManualExplosion = Micro.unwrapTile(function (tile) {
    return tile >= Tile.TINYEXP && tile <= Tile.LASTTINYEXP;
});
Micro.isRail = Micro.unwrapTile(function (tile) {
    return tile >= Tile.RAILBASE && tile < Tile.RESBASE;
});
Micro.isResidential = Micro.unwrapTile(function (tile) {
    return tile >= Tile.RESBASE && tile < Tile.HOSPITALBASE;
});
Micro.isRoad = Micro.unwrapTile(function (tile) {
    return tile >= Tile.ROADBASE && tile < Tile.POWERBASE;
});
Micro.normalizeRoad = Micro.unwrapTile(function (tile) {
    return (tile >= Tile.ROADBASE && tile <= Tile.LASTROAD + 1) ? (tile & 15) + 64 : tile;
});

Micro.isCommercialZone = function (tile) {
    return tile.isZone() && Micro.isCommercial(tile);
};
Micro.isIndustrialZone = function (tile) {
    return tile.isZone() && Micro.isIndustrial(tile);
};
Micro.isResidentialZone = function (tile) {
    return tile.isZone() && Micro.isResidential(tile);
};
Micro.randomFire = function () {
    return new Micro.Tile(Tile.FIRE + (Random.getRandom16() & 3), Tile.ANIMBIT);
};
Micro.randomRubble = function () {
    return new Micro.Tile(Tile.RUBBLE + (Random.getRandom16() & 3), Tile.BULLBIT);
};
...
Micro.MonsterSprite = function (map, spriteManager, x, y) {
    ...
    } else if (y > Micro.worldToPix(map.height) / 2) {
        this.frame = 1;
    } else {
        this.frame = 4;
    }
        ...
        while (y--) {
            ...
            for (x = startX; x < maxX; x++) {
                ...
                if (tile.isZone()) {
                    ...
                    if (tile.isPowered()) {
                        this.sim.census.poweredZoneCount += 1;
                        this._map.powerData[id] = 1;
                    } else {
                        this.sim.census.unpoweredZoneCount += 1;
                        this._map.powerData[id] = 2;// this.sim.needPower.push(id);
                    }
                }
    ...
    doDisasters: function (census, messageManager) {
        ...
        if (Random.getRandom(this.DisChance[this._gameLevel])) {
            switch (Random.getRandom(8)) {
                ...
                case 7:
                case 8:
                    if (census.pollutionAverage > 60) this._spriteManager.makeMonster(messageManager);
                    break;
            }
        }
    },
...
Micro.copyFrom = function (sourceMap, sourceFn) {
    ...
    while (i--) this.data[i] = sourceMap.data[i].map(mapFn);
}
...
Micro.Simulation.prototype = {
    ...
    simulate: function () {
        ...
        switch (this.phaseCycle) {
            ...
            case 8:
                ...
                break;
            case 9:
                if (this.cityTime % Micro.CENSUS_FREQUENCY_10 === 0) {
                    this.census.take10Census(this.budget);
                }
                ;
                if (this.cityTime % Micro.CENSUS_FREQUENCY_120 === 0) {
                    this.census.take120Census(this.budget);
                }
                if (this.cityTime % Micro.TAX_FREQUENCY === 0) {
                    this.budget.collectTax(this.gameLevel, this.census, this.messageManager);
                    this.evaluation.cityEvaluation();
                }
                ;
                break;
            case 10:
                if ((this.simCycle % 5) === 0) {
                    Micro.decRateOfGrowthMap(this.blockMaps);
                }
                ;Micro.decTrafficMap(this.blockMaps);
                this.sendMessages();
                break;
            case 11:
            ...
            case 14:
                ...
                break;
            case 15:
                if ((this.simCycle % Micro.speedFireAnalysis[speedIndex]) === 0) Micro.fireAnalysis(this.blockMaps);
                this.disasterManager.doDisasters(this.census, this.messageManager);
                break;
        }
    ...
    sendMessages: function () {
        ...
        switch (this.cityTime & 63) {
            ...
            case 30:
                ...
                break;
            case 32:
                var zoneCount = this.census.unpoweredZoneCount + this.census.poweredZoneCount;
                if (zoneCount > 0) {
    ...
    updateFrontEnd: function () {
        ...
        if (this.valves.changed) {
            ...
            this.messageManager.sendMessage(Messages.VALVES_UPDATED, {
                residential: this.valves.resValve,
                commercial: this.valves.comValve,
        ...
        if (this.evaluation.changed) {
            this.messageManager.sendMessage(Messages.EVAL_UPDATED, {
                classification: this.evaluation.cityClass,
                population: this.evaluation.cityPop,
                score: this.evaluation.cityScore
            });
