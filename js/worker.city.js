...
var update = function () {
    ...
    transMessage({
        tell: "RUN",
        infos: Game.infos,
        tilesData: Game.map.tilesData,
        powerData: power,
        sprites: Game.spritesData
    });
        ...
        for (var i = 0, l = messages.length; i < l; i++) {
            ...
            switch (m.message) {
                ...
                case Messages.QUERY_WINDOW_NEEDED:
                    transMessage({tell: "QUERY", queryTxt: this.currentTool.getInfo()});
                    break;
                case Messages.DATE_UPDATED:
                    this.infos[0] = [TXT.months[m.data.month], m.data.year].join(' ');
                    break;
                case Messages.EVAL_UPDATED:
                    this.infos[1] = TXT.cityClass[m.data.classification];
                    this.infos[2] = m.data.score;
                    this.infos[3] = m.data.population;
                    break;
                case Messages.FUNDS_CHANGED:
                    this.infos[4] = m.data;
                    break;
                case Messages.VALVES_UPDATED:
                    this.infos[5] = m.data.residential;
                    this.infos[6] = m.data.commercial;
                    this.infos[7] = m.data.industrial;
                    break;
                default:
        ...
        if (this.currentTool !== null) {
            ...
            switch (this.currentTool.result) {
                case this.currentTool.TOOLRESULT_NEEDS_BULLDOZE:
                    tell = TXT.toolMessages.needsDoze;
                    break;
                case this.currentTool.TOOLRESULT_NO_MONEY:
                    tell = TXT.toolMessages.noMoney;
                    break;
                default:
                    tell = '&nbsp;';
